...
for itr in range(max_iters):
    ...
    for xb,yb in batches:
        ...
        # yb_idx = np.array([1,0,1,1,1])
        # forward
        layer1_output = forward(xb,params,'layer1',sigmoid)
        probs = forward(layer1_output,params,'output',softmax)
        # print(f"probs: {probs}")
        # print(f"weights layer1: {params['Woutput']}")
    ...
    if 'W' in k:
        ...
        for i in range(size_x):
            for j in range(size_y):
                # v references to the dict params; therefore update v to update params
                v[i, j] += eps

                layer1_output = forward(x, params,'layer1',sigmoid)
                probs = forward(layer1_output,params,'output',softmax)
